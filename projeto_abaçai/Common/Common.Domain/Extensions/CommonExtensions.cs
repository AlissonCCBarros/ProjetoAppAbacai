...
public static class CommonExtensions
{
    ...
    public static bool IsSenhaForte(this string senha)
    {
        var temLetra = Regex.IsMatch(senha, @"([A-Za-z])");
        var temNumero = Regex.IsMatch(senha, @"\d");
        var temEspecial = Regex.IsMatch(senha, @"[!@#$%^&*()_+\-=\[\]{};':" + @"\|,.<>\/?]+");
        return temLetra && temNumero && senha.Length >= 6;// && temEspecial;
    }
    #endregion
